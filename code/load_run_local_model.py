...
app = FastAPI(title="Local Llama-3.1-8B-Instruct")
...
def run_inference(messages: List[Dict[str, str]], temperature: float, max_tokens: int) -> Dict[str, Any]:
...
async def handle_any_payload(request: Request):
    try:
    ...
    try:
        ...
        if isinstance(body, dict):
            ...
            try:
                raw_tokens = body.get("max_tokens", body.get("max_new_tokens", 10000))
                max_tokens = int(raw_tokens) if raw_tokens is not None else 10000
            except: max_tokens = 10000
        ...
        return run_inference(messages, temperature, max_tokens)
